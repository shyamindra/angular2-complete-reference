...
export class SessionService {
    ...
    requestPin(mobileNumber: number) {
        return this._http.post(this._url + "request_pin", JSON.stringify({mobile_number: mobileNumber}), this.getRequestOptions())
            .map(res => res);
    }

    verifyPin(mobileNumber: number, otp: number): Observable<any> {
        // console.log("mobile:" + mobileNumber);
         return this._http.post(this._url + "verify_pin", JSON.stringify({mobile_number: mobileNumber, OTP: otp}), this.getRequestOptions())
            .map(res => res);
    }
    ...
    loginMobileUser(mobile: string): Observable<any>{
        return this._http.post(this._url + "register", JSON.stringify({mobile: mobile}), this.getRequestOptions())
            .map((res: Response) => res.json());
    }
